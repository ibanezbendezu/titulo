...
export class ProgrammingLanguage extends Language {
  ...
  public async loadLanguageModule(): Promise<TreeSitterLanguage> {
    if (this.name !== 'java') {
      throw new Error(`Language ${this.name} is not supported. Only Java is supported.`);
    }

    if (this.languageModule === undefined) {
      try {
        this.languageModule = await import('tree-sitter-java');
      } catch (error) {
        throw new LanguageError(
          `Could not find language module for ${this.name}, ` +
          `searched in tree-sitter-${this.name}`
        );
      }
    }
    return this.languageModule.default;
  }
